class BaumgartnerDataset
  ...
  def analyze_sliced_transitions(strftime_str, percentile, only_higher=true)
    ...
    CSV.read(transition_slice_manifest_file(strftime_str)).flatten.each do |file|
      NodeWalkerDiffs.perform_async(@method_suffix, file, percentile, strftime_str, cumulative_post_cutoff, only_higher)
    end
